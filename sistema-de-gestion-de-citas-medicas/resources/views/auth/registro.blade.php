...
<div class="bg-surface rounded-3xl p-8 md:p-10 shadow-2xl border border-border flex flex-col items-center w-full">
    ...
    <!-- Registration Form -->
    <form id="form_registro" method="POST" action="{{ route('registro') }}" class="w-full space-y-4">
        @csrf
        ...
        <div class="space-y-1">
            ...
            <div class="relative">
                ...
                <input type="text" id="txt_nombre" name="nombre" value="{{ old('nombre') }}" required autocomplete="name" placeholder="Escriba su nombre y apellidos" class="w-full pl-10 pr-4 py-2.5 bg-white border @error('nombre') border-danger @else border-border @enderror rounded-xl text-sm text-text-primary focus:outline-none focus:border-brand-emerald focus:ring-2 focus:ring-brand-emerald/20 transition-all">
            </div>
            @error('nombre')
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="space-y-1">
                ...
                <div class="relative">
                    <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-text-muted text-xl">calendar_month</span>
                    <input type="date" id="inp_fecha_nac" name="fecha_nacimiento" value="{{ old('fecha_nacimiento') }}" required max="{{ now()->subYears(18)->toDateString() }}" class="w-full pl-10 pr-4 py-2.5 bg-white border @error('fecha_nacimiento') border-danger @else border-border @enderror rounded-xl text-sm text-text-primary focus:outline-none focus:border-brand-emerald focus:ring-2 focus:ring-brand-emerald/20 transition-all">
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="space-y-1">
                ...
                <div class="relative">
                    <select id="sel_sexo" name="sexo" class="w-full appearance-none px-4 pr-10 py-2.5 bg-white border @error('sexo') border-danger @else border-border @enderror rounded-xl text-sm text-text-primary focus:outline-none focus:border-brand-emerald focus:ring-2 focus:ring-brand-emerald/20 transition-all cursor-pointer">
            ...
            <div class="space-y-1">
                <label for="txt_nss" class="text-xs font-semibold text-text-secondary block">NSS <span class="font-normal text-text-muted">(opcional)</span></label>
                <div class="relative">
                    <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-text-muted text-xl">health_and_safety</span>
                    <input type="text" id="txt_nss" name="nss" value="{{ old('nss') }}" placeholder="Núm. Seguro Social" pattern="[0-9]{11}" maxlength="11" inputmode="numeric" title="El NSS debe contener exactamente 11 dígitos numéricos" class="w-full pl-10 pr-4 py-2.5 bg-white border @error('nss') border-danger @else border-border @enderror rounded-xl text-sm text-text-primary focus:outline-none focus:border-brand-emerald focus:ring-2 focus:ring-brand-emerald/20 transition-all">
                </div>
                @error('nss')
                    <p class="text-xs text-danger mt-1 flex items-center gap-1"><span class="material-symbols-outlined text-sm">error</span>{{ $message }}</p>
                @enderror
        ...
        <div class="flex items-center gap-2 pt-2">
            <span class="material-symbols-outlined text-brand-emerald text-lg">contact_mail</span>
            <h2 class="text-[11px] font-bold uppercase tracking-wider text-text-secondary">Contacto</h2>
            <div class="h-px flex-1 bg-border"></div>
        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div class="space-y-1">
                ...
                <div class="relative">
                    ...
                    <input type="tel" id="txt_telefono" name="telefono" value="{{ old('telefono') }}" maxlength="10" required inputmode="numeric" autocomplete="tel" placeholder="10 dígitos" class="w-full pl-10 pr-4 py-2.5 bg-white border @error('telefono') border-danger @else border-border @enderror rounded-xl text-sm text-text-primary focus:outline-none focus:border-brand-emerald focus:ring-2 focus:ring-brand-emerald/20 transition-all">
                </div>
                @error('telefono')
                    <p class="text-xs text-danger mt-1 flex items-center gap-1"><span class="material-symbols-outlined text-sm">error</span>{{ $message }}</p>
                @enderror
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div class="space-y-1">
                ...
                <div class="relative">
                    ...
                    <input type="password" id="txt_password" name="password" required minlength="8" autocomplete="new-password" placeholder="Mínimo 8 caracteres" class="w-full pl-10 pr-11 py-2.5 bg-white border @error('password') border-danger @else border-border @enderror rounded-xl text-sm text-text-primary focus:outline-none focus:border-brand-emerald focus:ring-2 focus:ring-brand-emerald/20 transition-all">
            ...
            <div class="space-y-1">
                ...
                <div class="relative">
                    <span class="material-symbols-outlined absolute left-3 top-1/2 -translate-y-1/2 text-text-muted text-xl">lock</span>
                    <input type="password" id="txt_password_confirm" name="password_confirmation" required minlength="8" autocomplete="new-password" placeholder="Repite la contraseña" class="w-full pl-10 pr-11 py-2.5 bg-white border border-border rounded-xl text-sm text-text-primary focus:outline-none focus:border-brand-emerald focus:ring-2 focus:ring-brand-emerald/20 transition-all">
                    <button type="button" data-toggle-password="txt_password_confirm" aria-label="Mostrar u ocultar contraseña" class="absolute right-3 top-1/2 -translate-y-1/2 text-text-muted hover:text-brand-emerald transition-colors">
                        <span class="material-symbols-outlined text-xl">visibility</span>
                    </button>
                </div>
            </div>
        ...
        <!-- Submit Button -->
        <button type="submit" data-loading-text="Registrando..." class="w-full bg-brand-emerald hover:bg-emerald-700 text-white py-3.5 px-6 rounded-full font-bold text-sm shadow-md hover:shadow-lg active:scale-[0.99] transition-all flex items-center justify-center space-x-2 mt-4 disabled:opacity-80 disabled:cursor-not-allowed disabled:hover:bg-brand-emerald">
            <span data-btn-text>Registrar Paciente</span>
            <span data-btn-icon class="material-symbols-outlined text-xl">how_to_reg</span>
        </button>
    </form>
...
<script>
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const curp = document.getElementById('txt_curp');
        ...
        // Teléfono y NSS: solo dígitos
        [['txt_telefono', 10], ['txt_nss', 11]].forEach(([id, max]) => {
            const el = document.getElementById(id);
            if (el) el.addEventListener('input', () => { el.value = el.value.replace(/\D/g, '').slice(0, max); });
        });

        // Checklist de contraseña en vivo
        const pwd = document.getElementById('txt_password');
        const pwdConfirm = document.getElementById('txt_password_confirm');
        const marcar = (id, ok) => {
